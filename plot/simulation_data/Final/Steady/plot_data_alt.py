...
while i < 3:#number_of_uavs:
	...
	for row in csvreader:
		...
		time_passed.append(float(row[2]))
	if i == 0:
		plt.plot(time_passed, gamma, 'r')
		#plt.text(time_passed[50],gamma[50], str(time_passed[50]) + ',' + str(gamma[50]), 'r')
	elif i == 1:
		plt.plot(time_passed, gamma, 'g')
		#plt.text(time_passed[50],gamma[50], str(time_passed[50]) + ',' + str(gamma[50]), 'b')
	elif i == 2:
		plt.plot(time_passed, gamma, 'b')
		#plt.text(time_passed[50],gamma[50], str(time_passed[50]) + ',' + str(gamma[50]), 'g')
	i = i + 1
plt.ylabel("Parameterized particle position (m)")
plt.xlabel("Time (s)")
plt.grid()
plt.savefig('Graphs/particle_position.pgf')
...
def plot_paths3d():
	...
	#plt.grid()
	plt.savefig('Graphs/paths3D.pgf', bbox_inches='tight')
